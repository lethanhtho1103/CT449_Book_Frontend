...
const fetchData = () => {
  axios
    .get("http://localhost:8082/authentication/info/staff/" + ID_User)
    .then((res) => {
      ...
      imageUploadEdit.value = `http://localhost:8082/${res.data.Avatar}`;
    })
...
const handleSave = () => {
  ...
  axios
    .put("http://localhost:8082/authentication/edit/staff/" + ID_User, formData)
    .then((res) => {
      if (res.data.error) {
